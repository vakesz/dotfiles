...
install_packages() {
    ...
    # Add WSL-specific packages if running on WSL
...
install_docker() {
    # Check if we're in WSL2 and recommend Docker Desktop
    if grep -q microsoft /proc/version 2>/dev/null && grep -q "WSL2" /proc/version; then
        log "WSL2 detected - Docker Desktop for Windows is recommended"
        log "Skipping Docker Engine installation (use Docker Desktop instead)"
        return
    fi
...
finalize_setup() {
    ...
    if grep -q microsoft /proc/version 2>/dev/null; then
        log "Configuring Git and locale for WSL"
        ...
        # Add WSL convenience aliases to .zshrc
        if [ -f ~/.zshrc ] && ! grep -q "WSL-specific" ~/.zshrc; then
            cat >> ~/.zshrc << 'EOF'

# WSL-specific aliases and settings
alias open='wslview'
alias pbcopy='clip.exe'
alias pbpaste='powershell.exe -command "Get-Clipboard" 2>/dev/null'

# Add Windows programs to PATH (WSL2)
